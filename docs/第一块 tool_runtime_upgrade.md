...
新增 `error_type` 后，工具失败不再只是一个布尔状态，而是被拆成可识别的结构化错误类型：
...
这些错误类型让 Runtime 可以按失败原因采取不同策略：参数错误可以引导模型修正输入，超时错误可以对只读工具触发重试，权限拒绝则直接中止，避免继续执行危险操作。

新增 `metadata` 后，可以记录压缩、重试、原始输出长度等运行时信息，让一次工具调用不仅有输出结果，也保留可诊断、可复盘的执行上下文。

价值：

```text
让工具调用结果从简单字符串升级为结构化执行结果，为错误恢复、tracing、运行质量分析和后续审计提供基础。
```
