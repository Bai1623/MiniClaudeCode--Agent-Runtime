...
### 第三层：ProjectIndex 和 Summarizer 项目索引与摘要
...
### 第五层：Harness、GitWorkflow 和 CLI 工程接入
...
```text
自动创建 project.md、files、decisions、tasks、context 存储结构
使用 Markdown 展示记录，使用内嵌 JSON 元数据保证结构化数据保真
支持项目摘要、文件摘要、工程决策、任务记忆的写入和读取
使用安全文件名和短哈希避免路径字符问题及文件名冲突
使用临时文件替换保证单条记录原子写入
list 方法按业务标识稳定排序
损坏记录返回包含文件路径的明确错误
```

第三层 ProjectIndex 和 Summarizer 已完成：
...
```text
Git 仓库优先扫描 tracked 和未忽略的 untracked 文件
非 Git 目录自动回退到确定性文件系统扫描
过滤 .git、.miniclaudecode、虚拟环境、缓存目录和 .env 文件
识别 Python、Markdown、TOML、JSON、配置与常见前端文本文件
计算 SHA-256、真实字节数和 UTC 修改时间
基于文件哈希判断已有 FileSummary 是否过期
限制索引路径必须位于项目根目录内，并跳过符号链接
Python 文件通过 AST 提取顶层 class、def、async def 和模块说明
Markdown 文件提取层级标题
普通文本生成限定行数和字符数的预览
二进制及非 UTF-8 文件只记录安全说明，不写入原始内容
摘要生成前校验缓存指纹，文件变化时自动刷新，避免旧哈希关联新内容
```

第四层 ContextBuilder 上下文构建与压缩已完成：
...
```text
根据当前任务关键词对 file summary、decision、task memory 做确定性相关性选择
项目摘要会优先进入上下文，保证 Agent 先拿到项目级认知
文件摘要按路径、语言、符号和摘要内容命中排序
工程决策会结合任务关键词和 architecture、runtime、harness、git、memory 等工程关键词排序
任务记忆按相关性优先，再按任务 id 新近程度排序
ContextBundle 作为结构化结果，render 方法输出稳定 Markdown
通过 max_chars 控制上下文字符预算，超出时依次裁剪任务记忆、工程决策和文件摘要
极小预算下会截断项目摘要，保证最终渲染内容不超过限制
```
...
能选择相关 file summary
能优先保留近期 task memory
能让相关 task memory 优先于更近但无关的记录
能选择关键工程 decision
能稳定渲染 Markdown
能限制最大字符数
非法 max_chars 会返回明确错误
```

第五层 Harness、GitWorkflow 和 CLI 工程接入已完成：

```text
Harness、GitWorkflow 和 CLI 工程接入
```
...
miniclaudecode/memory/store.py
tests/test_cli.py
tests/test_harness_task_harness.py
...
```text
CLI 新增 --memory-index，用于刷新项目级摘要和文件摘要缓存
CLI 新增 --memory-context "当前任务"，用于构建任务相关上下文并写入 .miniclaudecode/memory/context/latest.md
CLI 新增 --list-memory，用于查看当前已有 memory 记录数量
Harness 运行结束后可写入 TaskMemory，记录目标、任务结果、检查项和关键产物路径
Harness final_report.md 会记录本次写入的 memory 路径
GitWorkflowReport 支持转换为 TaskMemory，沉淀本次工程闭环的变更文件、测试命令、结果和提交信息
--git-summary 会把 GitWorkflow 分析结果写入 memory，后续 ContextBuilder 可以复用
MemoryStore 新增 write_context，用于保存渲染后的上下文 Markdown
```
...
```text
.miniclaudecode/memory/project.md
.miniclaudecode/memory/files/
.miniclaudecode/memory/tasks/
.miniclaudecode/memory/context/latest.md
.miniclaudecode/runs/<run_id>/final_report.md
```
