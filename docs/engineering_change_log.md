...
## 2026-09-18：多 trial 隔离编排

新增批次 Eval runner 和 `--trials N` 入口。同一案例的每次 trial 都创建独立 Agent、临时 Git 仓库和证据目录，并采用批次内稳定编号；某次基础设施故障不会污染或阻断后续 trial。批次汇总显式区分通过、任务失败和基础设施错误，为下一步计算 pass@k、成本和延迟指标提供可靠输入。
